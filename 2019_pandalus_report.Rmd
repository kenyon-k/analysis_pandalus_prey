...
library(ggtext)             # allows for text formatting in ggplots (Markdown syntax)
...
```{r study-area-map, echo=FALSE, fig.cap="*Location of the Northern and Striped Shrimp Fishing Areas (SFAs) 0-6, with the Western Assessment Zone (WAZ corresponding roughly to SFA 3) and Eastern ASsessment Zone (EAZ corresponding roughly to SFA 2) highlighted in green and blue, respectively. The 2019 NSRF-DFO survey areas are highlighted in blue, green, and orange.*"}

# If we don't include ArcGIS to R code here, then we will want to write code to import an image here. Otherwise the Figure caption/reference will not appear
...
-->
```{r sample-map, echo=FALSE, fig.cap="*Stomach sampling locations in the Western ASsessment Zone (green), Eastern ASsessment Zone (blue), and Shrimp Fishing Area 4 (orange) including the 200 and 500 m isobaths.*"}
...
Table\@ref(tab:test-captions) is piped into Markdown

```{r test-captions, file = "scripts/Script_Table02_Stomachs-Full-Empty.R", echo=FALSE}

# before knitting ensure to comment out in source code:
    # loading libraries
    # importing xxxx base dataframes (below loading libraries)
    # importing table dataframe (right before creating the table itself)
    # code that produces console outputs except for the final table
        # e.g. str()

# before knitting update the year on any source code outputs:
    # xxxxxxxxxxxxxxxxxxx there are 2 'write.csv' codes

```


Table written within Markdown

```{r stomachs full-empty, echo=FALSE}

# {file = 'Script_Table02_Stomachs-Full-Empty.R'}
...
#  set_caption(caption = "Number of full, empty, and total (full and empty) stomachs collected for Atlantic Cod (Gadus morhua), Greenland Halibut (Reinhardtius hippoglossides), redfishes (Sebastes sp.), and skates (Rajidae) within each size class.")  #%>%
...
```{r shrimp-percent, echo = FALSE, file = "scripts/Script_Figure06_Padalus-percent.R", fig.height = 6, fig.width = 12, fig.cap="*Relative contribution by (a)  percent weight (%W) and (b) percent number (%N) of *P. borealis*, *P. montagui*, and unidentifiable *Pandalus sp.* within Arctic Cod (*Gadus morhua*), Greenland Halibut (*Reinhardtius hippoglossides*), redfish (*Sebastes sp.*), and skate (*Rajidae*) stomachs in comparison to other taxonomic categories of prey.*"}
...
There are a few formatting options for Figure 6

```{r option-B, echo = FALSE, fig.height = 6, fig.width = 12, fig.cap="*Fig 6 Option B*"}

# This creates an outside border
...
```{r option-C, echo = FALSE, fig.height = 6, fig.width = 12, fig.cap="*Fig 6 Option C*"}
...
```{r option-D, echo = FALSE, fig.height = 6, fig.width = 12, fig.cap="*Fig 6 Option D*"}
...
```{r cod-shrimp-percent, echo = FALSE, file = "scripts/Script_Figure08_Padalus-percent-Cod.R", fig.height = 4, fig.width = 7,  fig.cap="*Relative contribution of *P. borealis*, *P. montagui*, and unidentifiable *Pandalus sp.* by (a) weight (%W) and (b) number (%N) found within  Arctic Cod (*Gadus morhua*) stomachs in comparison to other taxonomic categories of prey*"}
...
Below are formatting options for Fig 8:

```{r f8-option-B, echo = FALSE, fig.height = 4, fig.width = 7, fig.cap="*Fig 8 Option B*"}
...
```{r f8-option-C, echo = FALSE, fig.height = 4, fig.width = 7, fig.cap="*Fig 8 Option C*"}
...
```{r f8-option-D, echo = FALSE, fig.height = 4, fig.width = 7, fig.cap="*Fig 8 Option D*"}
